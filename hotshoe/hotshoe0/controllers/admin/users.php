...
class Users extends CI_Controller {
	...
	public function index()
	{
        ...
        $config['total_rows'] = $this->Hotshoe_model->countUsers();
		...
		$this->data['users'] = $this->Hotshoe_model->getUsers($result_per_page, $this->uri->segment(3)); 
	...
	public function editUser()
	{
		...
		$this->data['users'] = $this->Hotshoe_model->getUser($this->uri->segment(4)); 
		//Load the view
		$this->data['header'] = $this->load->view('admin/header', $this->data, true);
		$this->data['footer'] = $this->load->view('admin/footer', '', true);
	...
	public function edited()
	{
		...
		if($this->form_validation->run() == FALSE) {
			...
			//Validation passed
			//Update the user
			$this->Hotshoe_model->updateUser($this->uri->segment(4));
			//Return to user list
			redirect('/admin/users', 'refresh');
	  	}
	}
	...
	public function delete()
	{
		...
		$this->Hotshoe_model->removeUser($this->uri->segment(4));
		//Return to user list
		redirect('/admin/users', 'refresh');
	}
	...
	public function forgot()
	{
		...
		else
		{
			...
			$this->db->update('hotshoe_user', $data);
	...
	{
		$query = $this->db->get_where('hotshoe_user', array('email' => $str), 1);
		if ($query->num_rows()== 1)
		{
			return true;
		}
	...
	public function getPassword()
	{
		...
		$query=$this->db->get_where('hotshoe_user', array('rs' => $rs), 1);
      	...
      	else
      	{
			...
			else
			{
				...
				else
				{
					...
					$where->update('hotshoe_user',$data);
					$this->data['header'] = $this->load->view('admin/headerlog', $this->data, true);
					$this->data['footer'] = $this->load->view('admin/footer', '', true);
